...
> 🚧 Results images are generated by running the notebook. The default `MAX_EPOCHS = 30` is a fast sanity run — increase it (100–200 epochs) for a stronger score. After running in Colab, the notebook saves `assets/training_curves.png` and `assets/prediction.png`; commit those two files and they will render here as a training-curve plot and a ground-truth-vs-prediction overlay.
